...
class DmesgApp1(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.create_container_button1.setIcon(QtGui.QIcon('linux.png'))
        self.create_container_button1.setIconSize(QtCore.QSize(128, 128))
        self.create_container_button1.setText("normal")
        ...
        self.create_container_button2.setIcon(QtGui.QIcon('linux.png'))
        self.create_container_button2.setIconSize(QtCore.QSize(128, 128))
        self.create_container_button2.setText("vkernel")
    ...
    def create_container(self, flag):
        ...
        if flag:
            ...
            command = (f"docker run --rm -d --name {self.name2} -p {self.port}:22 --privileged "
                       f"--runtime=vkernel-runtime"
                       f"{test_image}")
        else:
        ...
        if flag:
            self.execdmesg_app = ExecDmesgApp(self.name2, self.port)
        else:
            self.execdmesg_app = ExecDmesgApp(self.name1, self.port)
        self.execdmesg_app.show()
...
class DmesgApp(QtWidgets.QWidget):
    ...
    def create_container(self, flag):
        if flag:
            ...
            command = (f"docker run --rm -d --name {self.name2} -p {self.port+1}:22 --privileged "
                       f"--runtime=vkernel-runtime"
                       f"{test_image}")
        else:
            self.name1 = f"normal{self.port}"
...
class CveApp(QtWidgets.QWidget):
    ...
    def create_container(self):
        if self.flag:
            self.name2 = "vkernel"
            command = (f"docker run --rm -d --name {self.name2} -p 3333:22 --privileged --runtime=vkernel-runtime "
                       f"{test_image}")
        else:
            self.name1 = "cve"
    ...
    def update_button_icon(self):
        if self.flag:
            ...
            self.button2.setText("unlock")
        else:
            self.button2.setIcon(QtGui.QIcon('open.png'))
            self.button2.setText("lock")
...
class ContainerCommandApp(QtWidgets.QWidget):
    def __init__(self, name, flag):
        ...
        buttons = [
            ...
            {"text": "create file", "icon": "file.png", "command": "touch /tmp/test_dir/key.txt"},
            {"text": "process status", "icon": "ps.png", "command": "ps"},
            {"text": "list", "icon": "list.png", "command": "ls -l"},
            {"text": "print working dir", "icon": "pwd.png", "command": "pwd"}
        ]
